...
class EmbedScript:
    ...
    def new_manifest_text(self, folder, macro):
        ...
        for path in ['Scripts/', 'Scripts/python/', proj_name_dir]:
            new_line += ' <manifest:file-entry manifest:full-path="%s" manifest:media-type="application/vnd.sun.star.framework-script"/>\n' % path

        p = proj_name_dir + macro
        new_line += ' <manifest:file-entry manifest:full-path="%s" manifest:media-type="application/binary"/>\n' % p
    ...
    def pack_script(self):
        ...
        macro_src = os.path.join(self.pydir, self.config.get('directories', 'source_dir'), self.app + '.py')
        macro_dest = os.path.join(extract_dir, 'Scripts', 'python', self.app, self.app + '.py')
        ...
        for file in os.listdir(self.pydir):
            if file.endswith(".odt"):
                ...
                self.logger.info('extracted ' + file_name + ' in temp dir ' + extract_dir)
                ...
                new_odf = os.path.join(self.pydir, file_name_parts[0] + '_MACRO.' + file_name_parts[1])
