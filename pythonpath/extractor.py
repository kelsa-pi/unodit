...
escape_components = ['bulletinboard', 'styles', 'style', 'menupopup', 'menuitem', 'radiogroup', 'event', 'title']
...
class ContextGenerator:
    ...
    def iter_element(self, tree, namespace):
        ...
        for node in tree.iter():
            if node.tag == component_ns:
                component_properties = {'{http://openoffice.org/2000/dialog}model': namespace}
    ...
    def iterfind_element(self, tree, namespace, nsmap):
        ...
        for one in tree.iterfind(ns, namespaces=nsmap):
            # level_one = {}
            current_component_name = one.get('{http://openoffice.org/2000/dialog}id')
            ...
            for two in one:
                ...
                for three in two:
                    ...
                    if current_component == 'menulist' or current_component == 'combobox':
                        val = three.attrib['{http://openoffice.org/2000/dialog}value']
            ...
            new_element[current_component_name]['{http://openoffice.org/2000/dialog}model'] = namespace
        return new_element
        ...
        for component, values in xdl_ctx.items():
            ...
            for xdl_prop, xdl_value in values.items():
                if xdl_prop not in current_uno_component:
                    if xdl_prop != 'model':
                        s = current_xdl_component + ' / ' + current_xdl_component_name
                        s = s.ljust(25)
                        maping += s + ":  '" + xdl_prop + "': {'name': '', 'type': ''},\n"
            all_maping += maping
            maping = ''
                ...
                if xdl_prop == 'model':
                    ...
                    try:
                        ...
                        if uno_prop == '' or uno_type == '':
                            ...
                            xdl_value = str(current_uno_component[xdl_prop]['special'][xdl_value])

                            component_uno_properties[uno_prop] = self._value_type(xdl_value, uno_type)
                        else:
    ...
    def _value_type(self, val, u_type):
        ...
        if u_type == 'string':
            ...
            if dy.startswith('0'):
                dy = dy[1]
            uno_value = ' uno.createUnoStruct(' + '"com.sun.star.util.Date"' + ', Year = ' + yr + ', Month = ' + mn + ', Day = ' + dy + ') '
        elif u_type == 'time':
            # 14050300
            hr = val[:2]
            mn = val[2:4]
            ...
            uno_value = ' uno.createUnoStruct(' + '"com.sun.star.util.Time"' + ', Hours = ' + hr + ', Minutes = ' + mn + ', Seconds = ' + sc + ', NanoSeconds = ' + ms + ', IsUTC = True' + ') '
